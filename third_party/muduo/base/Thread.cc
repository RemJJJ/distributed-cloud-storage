...
struct ThreadData {
    typedef fileserver::Thread::ThreadFunc ThreadFunc;
    ...
    /// @brief 运行线程函数
    void runInThread() {
        *tid_ = fileserver::CurrentThread::tid();
        tid_ = NULL;
        latch_->countDown();
        latch_ = NULL;

        fileserver::CurrentThread::setName(name_.empty() ? "fileserverThread"
                                                         : name_.c_str());
        ::prctl(PR_SET_NAME, fileserver::CurrentThread::name());

        try {
            func_();
            fileserver::CurrentThread::setName("finished");
        } catch (const Exception &ex) {
            fileserver::CurrentThread::setName("crashed");
