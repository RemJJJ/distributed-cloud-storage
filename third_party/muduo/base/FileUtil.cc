...
int FileUtil::ReadSmallFile::readToString(size_t maxSize, string *content,
                                          int64_t *fileSize,
                                          int64_t *modifyTime,
                                          int64_t *createTime) {
            ...
            if (::fstat(fd_, &statbuf) == 0) {
                if (S_ISREG(statbuf.st_mode)) {
                    *fileSize = statbuf.st_size;
                    content->reserve(
                        std::min(maxSize, static_cast<size_t>(*fileSize)));
...
bool FileUtil::readFile(StringArg filename, string *content, size_t sizeLimt) {
