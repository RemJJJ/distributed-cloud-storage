...
struct TimeZone::Data {
    ...
    void addTransition(time_t when, int localIdx) {
        ...
        transitions.push_back(
            Transition(when, when + local.gmtOffset, localIdx));
    }
...
TimeZone::Data::findLocalTime(time_t localtime) const {
    ...
    if (transitions.empty() || localtime < transitions.front().localtime) {
        ...
        auto trans =
            std::upper_bound(transitions.begin(), transitions.end(), sentry,
