...
const int kPollTimeMs = 1; // 将超时时间从10ms改为1ms，以更快地相应写事件
...
TimerId EventLoop::runAfter(double delay, TimerCallback cb) {
    Timestamp time(fileserver::addTime(Timestamp::now(), delay));
    return runAt(time, std::move(cb));
}

TimerId EventLoop::runEvery(double interval, TimerCallback cb) {
    Timestamp time(fileserver::addTime(Timestamp::now(), interval));
