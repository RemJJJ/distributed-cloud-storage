...
RouteMatch RouteTrie::findRoute(const std::string &path,
    ...
    std::string basePath =
        (queryPos != std::string::npos) ? path.substr(0, queryPos) : path;
    ...
    if (!matches.empty()) {
        ...
        for (const auto &match : matches) {
            if (match.first->isLeaf && match.first->handlers.find(method) !=
                                           match.first->handlers.end()) {
                LOG_INFO << "Found matching route with handler: "
                         << match.first->handlers[method];
...
void RouteTrie::findMatches(
    std::shared_ptr<TrieNode> node, const std::vector<std::string> &segments,
    size_t currentIndex,
    ...
    if (node->children.find("*") != node->children.end()) {
        ...
        if (!paramNode->paramNames.empty()) {
            ...
            findMatches(paramNode, segments, currentIndex + 1, newParams,
                        matches);
        }
    }
