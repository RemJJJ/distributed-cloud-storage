...
TcpConnection::TcpConnection(EventLoop *loop, const string &nameArg, int sockfd,
    ...
    : loop_(loop), name_(nameArg), state_(kConnecting), reading_(true),
      ...
      highWaterMark_(64 * 1012 * 1024) // 64MB
{
    // 设置通道回调函数
    ...
    if (state_ == kConnected) {
        if (loop_->isInLoopThread()) {
            ...
            loop_->runInLoop(std::bind(
                fp, this, StringPiece(static_cast<const char *>(data), len)));
        }
    }
}
...
void TcpConnection::sendInLoop(const void *data, size_t len) {
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0) {
        ...
        if (nwrote >= 0) {
            ...
            LOG_DEBUG << "sendInLoop: wrote " << nwrote << " bytes, remaining "
                      << remaining << " bytes";
    ...
    if (!faultError && remaining > 0) {
        ...
        LOG_DEBUG << "sendInLoop: append " << remaining
                  << " bytes to output buffer, oldLen = " << oldLen;
...
void TcpConnection::handleWrite() {
    loop_->assertInLoopThread();
    if (channel_->isWriting()) {
        LOG_DEBUG << "handleWrite: try to write "
                  << outputBuffer_.readableBytes() << " bytes";
        ...
        if (n > 0) {
            ...
            if (outputBuffer_.readableBytes() == 0) {
                LOG_DEBUG
                    << "handleWrite: output buffer empty, disable writing";
                ...
                LOG_DEBUG << "handleWrite: still have "
                          << outputBuffer_.readableBytes() << " bytes to write";
                // 继续尝试写入
                loop_->queueInLoop(
        ...
        LOG_ERROR << "Connection fd = " << channel_->fd()
