...
class HttpUploadHandler {
  private:
    ...
    bool initDatabase() {
        ...
        if (!mysql_real_connect(mysql, dbHost.c_str(), dbUser.c_str(),
                                dbPassword.c_str(), dbName.c_str(), dbPort,
                                NULL, 0)) {
  ...
  public:
    HttpUploadHandler(int numThreads, const std::string &dbHost = "localhost",
        ...
        : threadPool_("UploadHandler"), uploadDir_("uploads"),
          ...
          dbName(dbName), dbPort(dbPort), mysql(NULL) {
    ...
    void onConnection(const TcpConnectionPtr &conn) {
        if (conn->connected()) {
            ...
            if (auto context =
                ...
                if (auto uploadContext =
                        context->getContext<FileUploadContext>()) {
        ...
        try {
            ...
            for (const auto &route : routes_) {
                ...
                if (std::regex_match(path, matches, route.pattern)) {
                    ...
                    for (size_t i = 0;
                         i < route.params.size() && i + 1 < matches.size();
                         i++) {
  ...
  private:
    ...
    bool handleFileUpload(const TcpConnectionPtr &conn, HttpRequest &req,
            ...
            try {
                ...
                if (!headerFilename.empty()) {
                    ...
                    if (std::regex_search(body, matches, filenameRegex) &&
                        ...
                        LOG_INFO << "Using default filename: "
                                 << originalFilename;
                    }
                }
        ...
        if (uploadContext->getState() == FileUploadContext::State::kComplete ||
            ...
            std::string query =
                ...
                std::to_string(fileSize) + ", '" + escapeString(fileType) +
    ...
    bool handleDownload(const TcpConnectionPtr &conn, HttpRequest &req,
        ...
        if (filename.empty()) {
            sendError(resp, "Missing filename", HttpResponse::k400BadRequest,
                      conn);
            return true;
        }
        ...
        // 检查用户是否已登录
        std::string sessionId = req.getHeader("X-Session-ID");
        // 如果请求投中没有sessionId， 尝试从URL查询参数中获取
        if (sessionId.empty()) {
        ...
        if (!shareCode.empty()) {
            ...
            // 直接访问(需要登录)
            if (!isAuthenticated) {
                sendError(resp, "请先登录", HttpResponse::k401Unauthorized,
                          conn);
                return true;
            }
            query = "SELECT f.id, f.filename, f.original_filename, f.user_id, "
                    "NULL as share_type, NULL as shared_with_id, NULL as "
                    "extract_code "
        ...
        try {
            if (!fs::exists(filepath) || !fs::is_regular_file(filepath)) {
                sendError(resp, "File not found", HttpResponse::k404NotFound,
                          conn);
                return true;
            }
            ...
            if (!downContext) {
                // 创建新的下载上下文
                downContext = std::make_shared<FileDownContext>(
                    filepath, originalFilename);
            ...
            sendError(resp, "Download failed",
                      HttpResponse::k500InternalServerError, conn);
            return true;
        }
    }
    ...
    static void sendError(HttpResponse *resp, const std::string &message,
        ...
        json response = {{"code", static_cast<int>(code)},
                         {"message", message}};
    ...
    bool handleRegister(const TcpConnectionPtr &conn, HttpRequest &req,
        ...
        try {
            ...
            if (result && mysql_num_rows(result) > 0) {
                mysql_free_result(result);
                sendError(resp, "用户名已存在", HttpResponse::k400BadRequest,
                          conn);
                return true;
            }
    ...
    void initRoutes() {
        ...
        addRoute("/files", HttpRequest::kGet,
                 &HttpUploadHandler::handleListFiles);
        ...
        addRoute("/share", HttpRequest::kPost,
                 &HttpUploadHandler::handleShareFile);
        addRoute("/users/search", HttpRequest::kGet,
                 &HttpUploadHandler::handleSearchUsers);
        addRoute("/logout", HttpRequest::kPost,
                 &HttpUploadHandler::handleLogout);
    }
    ...
    void addRoute(const std::string &path, HttpRequest::Method method,
        ...
        routes_.emplace_back(pattern, std::vector<std::string>(), handler,
                             method);
    }
    // 带参数的路由
    ...
    bool handleSearchUsers(const TcpConnectionPtr &conn, HttpRequest &req,
        ...
        if (query.find("keyword=") == 0) {
            ...
            sendError(resp, "搜索关键词不能为空", HttpResponse::k400BadRequest,
                      conn);
    ...
    bool handleShareFile(const TcpConnectionPtr &conn, HttpRequest &req,
        ...
        try {
            ...
            std::string shareType = requestData["shareType"];
            ...
            if (shareType == "user" && requestData.contains("sharedWithId")) {
                // 指定用户分享
                sharedWithId =
                    std::to_string(requestData["sharedWithId"].get<int>());
                std::cout << sharedWithId << std::endl;
                // 检查是否以及分享给该用户
                std::string checkQuery =
                    "SELECT 1 FROM file_shares WHERE file_id = " +
                    std::to_string(fileId) +
            ...
            std::string insertQuery =
                ...
                escapeString(shareCode) + "', " + extractCode + ", " +
                expireStr + ")";
    ...
    bool handleShareAccess(const TcpConnectionPtr &conn, HttpRequest &req,
        ...
        if (!std::regex_search(path, matches, codeRegex) ||
            matches.size() < 2) {
            sendError(resp, "无效的分享链接", HttpResponse::k400BadRequest,
                      conn);
        ...
        if (req.getHeader("X-Requested-With") == "XMLHttpRequest" ||
            ...
            std::string query =
                ...
                "' "
                "AND (fs.expire_time IS NULL OR fs.expire_time > NOW()) "
                "AND (fs.share_type != 'protected' OR (fs.share_type = "
                "'protected' AND fs.extract_code = '" +
            ...
            bool isOwner =
                (row[13] &&
                 std::stoi(row[13]) == userId); // f.user_id == current_user_id
            int sharedWithId =
                row[3] ? std::stoi(row[3]) : 0; // fs.shared_with_id
            std::string dbExtractCode = row[5] ? row[5] : "";
    ...
    bool handleShareDownload(const TcpConnectionPtr &conn, HttpRequest &req,
        ...
        LOG_INFO << "shareCode = " << shareCode
                 << ", extractCode = " << extractCode;
        if (shareCode.empty()) {
            sendError(resp, "Missing share code", HttpResponse::k400BadRequest,
                      conn);
            return true;
        }
        ...
        if (!result || mysql_num_rows(result) == 0) {
            if (result)
                mysql_free_result(result);
            LOG_ERROR << "分享不存在或已过期";
            sendError(resp, "Share not found or expired",
                      HttpResponse::k404NotFound, conn);
            return true;
        }
